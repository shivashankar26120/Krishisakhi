...
def _load_llm(settings) -> str | None:
    ...
    Actual generation happens through Hugging Face Inference Providers
    (serverless). The `provider` kwarg was introduced in huggingface_hub
    0.27.0 — we detect the version and omit it for older installs.

    HF_TOKEN is stripped of whitespace/newlines before use to prevent
    invalid HTTP header errors (seen in Piper and HF download logs).
    """
